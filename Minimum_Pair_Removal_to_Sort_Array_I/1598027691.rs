...
impl Solution {
    pub fn minimum_pair_removal(mut nums: Vec<i32>) -> i32 {
        ...
        while !Self::is_sorted_non_decreasing(&nums) {
            ...
            let merged = nums[best_index] + nums[best_index + 1];
            nums.remove(best_index + 1);
            nums[best_index] = merged;
            steps += 1;
        }
    ...
    fn is_sorted_non_decreasing(arr: &[i32]) -> bool {
